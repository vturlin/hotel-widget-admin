...
// ─── Reassurance widget routes ──────────────────────────────────────
// Same path layout as lead-gen / stress, pointing at the
// reassurance-widget GitHub repo. Set GITHUB_REPO_REASSURANCE to
// override the default 'reassurance-widget'.

function reassuranceRepo() {
  return process.env.GITHUB_REPO_REASSURANCE || 'reassurance-widget';
}

app.get('/api/reassurance/current-config/:hotelId', async (req, res) => {
  try {
    const { hotelId } = req.params;
    if (!/^[a-zA-Z0-9_-]+$/.test(hotelId)) {
      return res.status(400).json({ error: 'Invalid hotelId' });
    ...
    console.error('[api/reassurance/current-config]', err);
    return res.status(500).json({ error: err.message });
  }
});

app.post('/api/reassurance/publish', async (req, res) => {
  try {
    const { hotelId, config } = req.body || {};
    if (!hotelId || typeof hotelId !== 'string') {
      return res.status(400).json({ error: 'Missing hotelId' });
    ...
    const contentB64 = Buffer.from(JSON.stringify(config, null, 2), 'utf-8').toString('base64');
    const payload = {
      message: `reassurance: ${sha ? 'update' : 'create'} ${hotelId}`,
      content: contentB64,
      branch,
    };
    if (sha) payload.sha = sha;
...
app.get('/api/reassurance/list-configs', async (req, res) => {
  try {
    if (!githubEnvOk(res)) return;
    const owner = process.env.GITHUB_OWNER;
    const repo = reassuranceRepo();
    ...
    const hotels = await Promise.all(
      jsonFiles.map(async (file) => {
        ...
        try {
          ...
          console.error(`[reassurance list-configs] failed for ${hotelId}:`, err.message);
          return { hotelId, hotelName: '', updatedAt: null, error: true };
        }
      })
    );
    ...
    console.error('[api/reassurance/list-configs]', err);
    return res.status(500).json({ error: err.message });
  }
});

app.post('/api/reassurance/duplicate', async (req, res) => {
  try {
    if (!githubEnvOk(res)) return;
    const { sourceId, newId } = req.body || {};
    if (!sourceId || !newId) {
    ...
    const putRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${encodeURIComponent(targetPath)}`, {
      ...
      headers: { ...githubHeaders(), 'Content-Type': 'application/json' },
      body: JSON.stringify({
        message: `reassurance: duplicate ${sourceId} → ${newId}`,
        content: Buffer.from(JSON.stringify(config, null, 2), 'utf-8').toString('base64'),
        branch,
      }),
    });
    ...
    console.error('[api/reassurance/duplicate]', err);
    return res.status(500).json({ error: err.message });
  }
});

app.delete('/api/reassurance/config/:hotelId', async (req, res) => {
  try {
    if (!githubEnvOk(res)) return;
    const { hotelId } = req.params;
    if (!/^[a-zA-Z0-9_-]+$/.test(hotelId)) {
    ...
    const delRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents/${encodeURIComponent(filePath)}`, {
      ...
      headers: { ...githubHeaders(), 'Content-Type': 'application/json' },
      body: JSON.stringify({
        message: `reassurance: delete ${hotelId}`,
        sha: fileData.sha,
        branch,
      }),
    });
    ...
    console.error('[api/reassurance/config DELETE]', err);
    return res.status(500).json({ error: err.message });
  }
});
